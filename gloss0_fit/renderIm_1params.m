...
sprintf('BRDF rho_s: %f and rho_d: %f', ro_s, ro_d)
% myparams = [ro_s, ro_d, alphau, light];
% setGlobal_oneparam(myparams);

T = cell2table(mycell, 'VariableNames', {'ro_s' 'ro_d' 'alphau' 'light'});
writetable(T,'/scratch/gk925/brdf_fitting_spray_LOG/gloss0_fit/sphere_3params_Conditions.txt','Delimiter','\t')
%% Rendering bit

% Set preferences
setpref('RenderToolbox3', 'workingFolder', '/scratch/gk925/brdf_fitting_spray_LOG/gloss0_fit');
...
mappingsFile = '0gloss_DefaultMappings.txt';
...
imPath = ['/scratch/gk925/brdf_fitting_spray_LOG/gloss0_fit/', hints.recipeName, '/renderings/Mitsuba/test_sphere-001.mat']
...
mask = zeros(1005,668);
mask(382:580,256:480)=1;

load('registered0.mat') % make this a variable
